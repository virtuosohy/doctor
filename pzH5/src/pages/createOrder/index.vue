...
const showPeopleColumns = computed(() =>{
  return createInfo.companion.map(item=>{
    return { text :item.name ,value:item.id }
  })
})

const showPeopleConfirm = (item) =>{
   form.companion_id =  item.selectedOptions[0].value
  peopleName.value =  item.selectedOptions[0].text
...
<van-cell-group class="cell">
  ...
  <van-cell >
    ...
    <van-cell-group title="服务需求" class="cell">
      <van-field
          ...
          style="height: 100px;"
          v-model="form.demand"
          placeholder="请简单填写你要就诊的科室 "
      />
    </van-cell-group>
    <van-button @click="submit" class="submit" type="primary" size="large">提交</van-button>
  ...
  >
    ...
    >
      <van-picker
          title="选择陪诊师"
          :columns="showPeopleColumns"
          @cancel="showPeople = false"
          @confirm="showPeopleConfirm"
      />
    </van-popup>
  </div>
</template>
